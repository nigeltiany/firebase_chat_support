...
module.exports = functions.database.ref('/messages/{User_ID}/{Message_ID}').onCreate(event => {
    ...
    /**
     * Stop Recursive Messages from this function
     */
    ...
    // A message reply
        ...
        admin.database().ref('/conversations/' + event.data.val().conversation_id + '/members').once('value', (recipients) => {
            recipients.val().map((recipient_id) => {
                // User already has a copy of the message they sent
                if(recipient_id === event.params.User_ID){
                    return
                }
                // Write the reply to each recipient. __response MUST be set to true to avoid recursion
                return admin.database().ref('messages/' + recipient_id).push().set(Object.assign({}, event.data.val(), {__response: true}))
            })
        })
    }
})